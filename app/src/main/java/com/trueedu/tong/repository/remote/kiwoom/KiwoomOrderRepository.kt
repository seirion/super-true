...
class KiwoomOrderRepository @Inject constructor(
    ...
    suspend fun placeOrder(account: BrokerAccount, request: OrderRequest): Result<OrderResult> = runCatching {
        ...
        val body = mapOf("acnt_no" to account.accountNum, "acnt_pw" to credentialStorage.getPassword(account.id), "stk_cd" to request.code, "ord_qty" to request.quantity.toString(), "ord_unpr" to request.price.toString(), "ord_dvsn" to if (request.isMarket) "3" else "0", "dmst_stex_tp" to "KRX")
