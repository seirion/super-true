...
class LsOrderRepository @Inject constructor(
    ...
    suspend fun placeOrder(account: BrokerAccount, request: OrderRequest): Result<OrderResult> = runCatching {
        ...
        val headers = mapOf("authorization" to "Bearer $token", "tr_cd" to "CSPAT00601", "tr_cont" to "N", "tr_cont_key" to "", "mac_address" to "")
        val body = mapOf("AcntNo" to account.accountNum, "Pwd" to "", "IsuNo" to request.code, "OrdQty" to request.quantity.toString(), "OrdPrc" to request.price.toString(), "BnsTpCode" to if (request.isBuy) "1" else "2", "OrdprcPtnCode" to if (request.isMarket) "03" else "00", "MgntrnCode" to "000", "LoanDt" to "")
